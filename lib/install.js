...
const install = async () => {
	try {
		...
		try {
			await binBuild.url('https://storage.googleapis.com/downloads.webmproject.org/releases/webp/libwebp-1.2.0.tar.gz', [
...
(async () => {
	try {
		...
		await fs.mkdir(vendor).catch(error => {
		...
		await fs.symlink(systemBin, target).catch(error => {
			if (error.code === 'EEXIST') {
				return;
			}
